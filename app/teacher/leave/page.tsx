...
import type { TeacherLeaveRequest } from '@/lib/types';

export default function TeacherLeavePage() {
  const [requests, setRequests] = React.useState<TeacherLeaveRequest[]>([]);
    ...
    const fetchRequests = async () => {
      ...
      if (user) {
        const { data } = await supabase
          ...
          .select('*, reviewer:profiles!teacher_leave_requests_reviewed_by_fkey(*)')
          .eq('teacher_id', user.id)
          .order('created_at', { ascending: false });

        setRequests((data as TeacherLeaveRequest[]) || []);
      }
      setLoading(false);
    };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your leave and absence requests
          </p>
        </div>
